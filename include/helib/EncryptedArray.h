...
public:
  ...
  {throw helib::LogicError("EncryptedArrayBase::decrypt for undefined type");}
  virtual void decrypt(const Ctxt& ctxt, const SecKey& sKey, PlaintextArray& ptxt) const
  {throw helib::LogicError("EncryptedArrayBase::decrypt for undefined type");}
  virtual void decrypt(const Ctxt& ctxt, const SecKey& sKey, std::vector<double>& ptxt) const
  {throw helib::LogicError("EncryptedArrayBase::decrypt for undefined type");}
...
public:
  explicit
  EncryptedArrayDerived(const Context& _context, const RX& _G,
...
private:
  ...
  template<class T>
  void genericDecrypt(const Ctxt& ctxt, const SecKey& sKey, 
                      T& array) const
  {
...
public:
  ...
  void encryptOneNum(Ctxt& ctxt, const PubKey& key, double num,
                     double useThisSize=-1, long precision=-1) const
  {
    //OLD: assert(&getContext() == &ctxt.getContext());
  ...
  template<class PTXT>
  void encrypt(Ctxt& ctxt, const PubKey& key, const PTXT& ptxt,
               double useThisSize, long precision=-1) const
  {
    //OLD: assert(&getContext() == &ctxt.getContext());
    helib::assertEq(&getContext(), &ctxt.getContext(), "Cannot decrypt when ciphertext has different context than EncryptedArray");
  ...
  encrypt(Ctxt& ctxt, const PubKey& key, const PTXT& ptxt) const
   ...
   **/
  template <typename Scheme>
  void decrypt(const Ctxt& ctxt, const SecKey& sKey, helib::Ptxt<Scheme>& ptxt) const
  {
...
EncryptedArrayBase*
buildEncryptedArray(const Context& context, const PAlgebraMod& alMod,
