...
template<typename T>
long countNonNull(T* ptrs[], long first, long last)
{
  long count = 0;
...
struct PtrVector_VecPt : PtrVector<T> {
  ...
  long numNonNull(long first=0, long last=LONG_MAX) const override
  {
    ...
    return countNonNull((T**)v.elts(), first, last);
  }
  T* ptr2nonNull() const override
  { return findNonNull((T**)v.elts(), v.length()); }
};
...
struct PtrVector_vectorPt : PtrVector<T> {
  ...
  long numNonNull(long first=0, long last=LONG_MAX) const override
  {
    ...
    return countNonNull((T**)v.data(), first, last);
  }
  T* ptr2nonNull() const override
  { return findNonNull((T**)v.data(), v.size()); }
};
...
struct PtrVector_VecT : PtrVector<T> {
  ...
  T* operator[](long i) const override {return (T*) &(v[i]);}
  long size() const     override { return v.length(); }
  ...
  T* ptr2nonNull() const override
  { return (T*)((v.length()>0)? &(v[0]) : nullptr); }
};
...
struct PtrVector_slice : PtrVector<T> {
  ...
  PtrVector_slice(PtrVector<T>& _orig, long from, long _sz=-1)
  ...
  T* ptr2nonNull() const override { return orig.ptr2nonNull(); }
};
