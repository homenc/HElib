                 ...
                 bool invert = false)
{
  switch (ea.getAlMod().getTag()) {
  case PA_GF2_tag: 
    return new AltStep1Matrix<PA_GF2>(ea, sig, reps, dim, cofactor, invert);

  case PA_zz_p_tag: 
    return new AltStep1Matrix<PA_zz_p>(ea, sig, reps, dim, cofactor, invert);
...
AltEvalMap::AltEvalMap(const EncryptedArray& _ea, const Vec<long>& mvec, bool _invert)
...
void AltEvalMap::apply(Ctxt& ctxt) const
{
  if (!invert) {
    // forward direction
