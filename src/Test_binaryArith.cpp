...
void testAddTwo(FHESecKey& secKey, long bitSize);
void testAddMany(FHESecKey& secKey, long bitSize);
void testProduct(FHESecKey& secKey, long bitSize);

int main(int argc, char *argv[])
{
  ArgMapping amap;
  long prm=2;
  amap.arg("prm", prm, "parameter size (0-tiny, 1-small, 2-big)");
  long bitSize = 5;
  ...
  for (long i=0; i<nTests; i++)
    testAddTwo(secKey, bitSize);

  for (long i=0; i<nTests; i++)
    testAddMany(secKey, bitSize);
  */
  for (long i=0; i<nTests; i++)
    testProduct(secKey, bitSize);
...
void testProduct(FHESecKey& secKey, long bitSize)
{
  ...
  multTwoNumbers(eep, CtPtrs_VecCt(enca), CtPtrs_VecCt(encb),
                 /*negative=*/false);
  decryptBinaryNums(slots, eep, secKey, ea);
  } // get rid of the wrapper
  if (slots[0] != pa*pb) {
    cout << "Product error: pa="<<pa<<", pb="<<pb
         << ", but product="<<slots[0]<<endl;
    exit(0);
  }
  else cout << "positive product succeeded: "
            << pa<<"*"<<pb<<"="<<slots[0]<<endl;
  ...
  else cout << "negative product succeeded: "
            << pa<<"*"<<pb<<"="<<slots[0]<<endl;
