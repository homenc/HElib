...
{
  FHE_NTIMER_START(__rotate_ctxt);
  ...
  if (al.numOfGens()==1) { // VJS: bug fix: <= must be ==
    rotate1D(ctxt, 0, amt);
    FHE_NTIMER_STOP(__rotate_ctxt);
    return;
  }

  // Make sure that amt is in [1,nslots-1]
  amt %= (long) al.getNSlots();
  if (amt == 0) { FHE_NTIMER_STOP(__rotate_ctxt); return; }
  ...
  if (al.SameOrd(i) || v==0) rotate1D(ctxt, i, v); // no need to optimize
    ...
    if (i <= 0) { FHE_NTIMER_STOP(__rotate_ctxt); return; }  // no more generators
