               ...
               long L, long m)
{
  ...
  DoubleCRT crt(poly, context, context.ctxtPrimes);

  const long NUM = 1000;
  long iter = 5;

  Vec<Ctxt*> ctxtvec(INIT_SIZE, NUM);
  Vec<ZZX*> polyvec(INIT_SIZE, NUM);
  Vec<DoubleCRT*> crtvec(INIT_SIZE, NUM);

  for (long i = 0; i < NUM; i++) {
    ctxtvec[i] = new Ctxt(c0);
    polyvec[i] = new ZZX(poly);
    crtvec[i] = new DoubleCRT(crt);
  }
  ...
  for (long j = 0; j < iter; j++) {
    for (long i = 0; i < NUM; i++) {
  #if 1
      ctxtvec[i]->multByConstant(*polyvec[i]);
  #else
      ctxtvec[i]->multByConstant(*crtvec[i]);
  #endif
    }
  }
