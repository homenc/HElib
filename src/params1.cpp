   ...
   for (long m = 1001; m <= 80000; m += 2) {
      ...
      for (long i = 0; i < k; i++) {
         for (long j = i+1; j < k; j++) {
            ...
            PAlgebra pal1(m1, p);
            if (pal1.numOfGens() > 2) continue;

            bool this_is_interesting = 
                 ((multOrd(p, fac1[i]) != d) && 
                  (multOrd(p, fac1[j]) != d) &&
                  (pal1.numOfGens() ==2));

            bool good = true;
            for (long w = 0; w < pal1.numOfGens(); w++)
               if (!pal1.SameOrd(w)) good = false;
            ...
            if (weighted_cost(cost, depth) < best_weighted_cost) {
               interesting = this_is_interesting;
               gen_index = i;
               gen_index2 = j;
               good_gen = good;
               first_gen = pal1.ZmStarGen(0);
      ...
      if (!interesting) continue; // just looking for interesting solutions
