...
/* Class KeySwitch - The key-switching matrix. 
 *
 ...
 *     bi = P*Bi*s' + p*ei - s\times ai mod P*q0,
 ...
 * (1,s) \cdot (q0,q1) = \sum_i Ri*(s\times ai-s\times ai+p*ei+P*Bi*s')
 *       = P * \sum_i Bi*Ri \times s' + p\sum_i Ri \times ei
 *       = P * R \times s' + p*a-small-element (mod P*q0)
...
public:
  ...
  void verify(FHESecKey& sk); // debug routine
...
/* Classes FHEPubKey/FHESecKey - The public and secret keys
 ********************************************************************/
class FHEPubKey { // The public key
  const FHEcontext& context; // The context

  Ctxt pubEncrKey;  // The public encryption key is an encryption of 0,
...
public:
  ...
  // Find a key-switching matrix by its indexes, if no such matrix
  // exists it returns a dummy matrix with toKeyID==-1.
  ...
  // Get the next matrix to use for multi-hop automorphism
  ...
  // Encryption, returns the plaintext-space that was actually used
...
public:
  ...
  // Q is the product of special primes, and the Bi's are the products of primes
  // in the i'th digit. The plaintext space defaults to 2^r, as defined by
  // context.mod2r.
  ...
  // This debugging version returns in f the polynomial before reduction
  // modulo the ptxtSpace
...
// A few "general purpose" startegies for deciding what automorphism
// key-switching matrices to generate.
