...
long addCoord(int i, int d, int offset, const EncryptedArray& ea,
              const vector<long>& prods)
{
  offset = offset % ea.sizeOfDimension(d);
  if (offset < 0) offset += ea.sizeOfDimension(d); 
  long i_d = ea.coordinate(d, i);

  long i_d1 = (i_d + offset) % ea.sizeOfDimension(d);

  long i1 = i + (i_d1 - i_d) * prods[d+1];
               ...
               long L, long m)
{
  ...
  vector<long> ptv;
  ptv.resize(nslots);
  for (long i = 0; i < nslots; i++) ptv[i] = i;

  Ctxt ctxt(publicKey);
  ea.encrypt(ctxt, publicKey, ptv);
  ea.rotate1D(ctxt, 0, 2);
  //             dim^

  vector<long> ptv1;
  ea.decrypt(ctxt, secretKey, ptv1);
  ...
  for (long i = ndims-1; i >= 0; i--)
    prods[i] = ea.sizeOfDimension(i)*prods[i+1];
  ...
  for (long i = 0; i < nslots; i++) iv[i] = i;

  for (long i = 0; i < nslots; i++)
    iv1[addCoord(i, 0, 2, ea, prods)] = iv[i];
    //           dim^
