...
struct Parameters {
    ...
    Parameters(long p, long r, long c, long L, long N, long skHwt, long nthreads, long seed, long useCache, int force_bsgs, int force_hoist) :
        p(p),
        r(r),
        c(c),
        L(L),
...
class GTest_bootstrapping : public ::testing::TestWithParam<Parameters> {
    protected:
        ...
        static constexpr std::size_t num_mValues = (sizeof(mValues)/(14*sizeof(long)));
...
TEST_P(GTest_bootstrapping, bootstrapping_works_correctly)
{
    ...
    for (long num=0; num<INNER_REP; num++) {  // multiple tests with the same key
