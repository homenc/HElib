...
struct Parameters {
    ...
    Parameters(long p, long r, long c, long L, long N, long t, long nthreads, long seed, long useCache, int force_bsgs, int force_hoist) :
        p(p),
        r(r),
        c(c),
        L(L),
  ...
  long t; // Hamming weight of recryption secret key
...
class GTest_ThinBootstrapping : public ::testing::TestWithParam<Parameters> {
    protected:
        ...
        static std::size_t getIdx(const long p, const long N)
        {
            ...
            throw std::invalid_argument("Could not find mValues row corresponding to p=" + std::to_string(p) + ", N=" + std::to_string(N));
        };
        ...
        static FHESecKey setUpSecretKey(FHESecKey& secKey)
        {
            ...
            t += NTL::GetTime();
            if (!helib_test::noPrint) std::cout << " done in "<<t<<" seconds\n";
            return secKey;
        }
        ...
        GTest_ThinBootstrapping() : 
            ...
            idx(getIdx(p, N)),
            phim(mValues[idx][1]),
            m(validatem(mValues[idx][2], p)),
            mvec(calculateMvec(idx)),
