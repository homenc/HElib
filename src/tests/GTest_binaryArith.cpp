...
struct Parameters {
    Parameters(long prm, long bitSize, long bitSize2, long outSize, bool bootstrap, long seed, long nthreads) :
        prm(prm),
        bitSize(bitSize),
        bitSize2(bitSize2),
        outSize(outSize),
...
class GTest_binaryArith : public ::testing::TestWithParam<std::tuple<Parameters, int>> {
    protected:
        ...
        static NTL::Vec<long> calculateMvec(const long *vals)
        {
            ...
            if (vals[5]>1) NTL::append(mvec, vals[5]);
            if (vals[6]>1) NTL::append(mvec, vals[6]);
            return mvec;
        };
...
TEST_P(GTest_binaryArith, fifteenForFour)
{
    ...
    if (helib_test::verbose) {
        std::cout << "15to4 succeeded, sum"<<inputBits<<"="<<sum2<<std::endl;
    }
};
...
TEST_P(GTest_binaryArith, product)
{
    ...
    if (helib_test::verbose) {
        ...
        if (outSize) std::cout << "bottom "<<outSize<<" bits of ";
        std::cout << pa<<"*"<<pb<<"="<<slots[0]<<std::endl;
    }
    ...
    decryptAndPrint((std::cout<<" after multiplication: "), *minCtxt, secKey, ea,0);
    std::cout << std::endl;
#endif
};
...
INSTANTIATE_TEST_SUITE_P(small_parameter_sizes_repeated, GTest_binaryArith, ::testing::Combine(
            ::testing::Values(
                //SLOW
