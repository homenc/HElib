...
::testing::AssertionResult filesAreEqual(std::string filename1, std::string filename2)
{
  std::ifstream file1(filename1);
  std::ifstream file2(filename2);
 
  if(!file1.is_open() && !file2.is_open()){
      throw std::runtime_error("Could not open one of the following files: " + filename1 + " and/or " + filename2 + ".");
  }
  ...
  if(file1size != file2size){ 
    file1.close();
    file2.close();
    return ::testing::AssertionFailure() << "Files " << filename1 << " and " << filename2 << " not the same size :(" << std::endl;
  }
  ...
  for(size_t i=file1size, cnt=0; i > 0; i-=curBlckSz, cnt++) {
    ...
    if(memcmp(buffer1, buffer2, curBlckSz)) {
        return ::testing::AssertionFailure() << "Block "
...
struct Parameters {
    Parameters(long m, long r, long p, long c, long L, const std::string& sampleFilePrefix, bool cleanup) :
        m(m),
        r(r),
        p(p),
        c(c),
...
TEST_P(GTest_Bin_IO, implements_binary_file_io_correctly)
{
    ...
    { // 2. Read in bin files and write out ASCII.
        if (!helib_test::noPrint) {
            std::cout << "Test to read binary file and write it out as ASCII" << std::endl;
        }
    ...
    { // 4. Read in binary and perform operation.
        if (!helib_test::noPrint) {
            std::cout << "Test reading in Binary files and performing an operation between two ctxts\n";  
        }
    ...
    { // 5. Read in binary from opposite little endian and print ASCII and compare
        ...
        std::string otherEndianFileIn
            = testResourcePath + sampleFilePrefix + (littleEndian? "_BE.bin" : "_LE.bin");
        std::string otherEndianASCII
            = testResourcePath + sampleFilePrefix + (littleEndian? "_BE.txt" : "_LE.txt");
        ...
        if(sampleFilePrefix.empty()) {
            if (!helib_test::noPrint)
                std::cout << "\tSample prefix not provided, test not done." << std::endl;
        } else {
            if (!helib_test::noPrint) {
            ...
            ASSERT_TRUE(inFile.is_open()) << "file " << otherEndianFileIn 
                        << " could not be opened.";
