...
struct Parameters {
    Parameters(long prm, long bitSize, long outSize, long nTests, bool bootstrap, long seed, long nthreads) : 
        prm(prm),
        bitSize(bitSize),
        outSize(outSize),
        nTests(nTests),
    ...
    protected:
                ...
                const long nthreads)
        {
            if (helib_test::verbose) {
                std::cout <<"input bitSize="<<bitSize<<", output size bound="<<outSize
