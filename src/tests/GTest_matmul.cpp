 ...
 *   MatMul1D* buildRandomMultiMatrix(const EncryptedArray& ea, long dim);
 test_common.h   BlockMatMul1D* buildRandomBlockMatrix(const EncryptedArray& ea, long dim);
...
struct Parameters {
    Parameters(long m, long p, long r, long L, long dim, long nt, long full, long block, std::vector<long> gens, std::vector<long> ords, int ks_strategy, int force_bsgs, int force_hoist) :
        m(m),
        p(p),
        r(r),
        L(L),
    ...
    {};
        ...
        virtual void SetUp()
        {
            ...
            if (helib_test::verbose) {
                ...
                if (!full)
                    std::cout << (ea.size()/ea.sizeOfDimension(dim))<<" products in parallel, ";
            }
...
// This is how we pack a matrix type and a reference to a Parameters object into a single type for gtest
template <class T, const Parameters& params> struct MatrixTypeAndParams {
