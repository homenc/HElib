...
TEST_P(GTest_approxNums, basic_arithmetic_works)
{
  ...
  EXPECT_TRUE(cx_equals(vd, vd1, epsilon));
}
...
#endif
  EXPECT_TRUE(cx_equals(realParts,real_dec,epsilon));
  EXPECT_TRUE(cx_equals(imParts,im_dec,epsilon));
}
...
TEST_P(GTest_approxNums, rotates_and_shifts_work)
{
  ...
  EXPECT_TRUE(cx_equals(vd1, vd_dec, epsilon));
}
...
INSTANTIATE_TEST_SUITE_P(typical_parameters, GTest_approxNums, ::testing::Values(
            //SLOW
            Parameters(1024, 8, 150, 0.01)
            //FAST
            //Parameters(128, 8, 150, 0.01)
            ));
