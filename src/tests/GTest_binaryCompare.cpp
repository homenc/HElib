...
TEST_P(GTest_binaryCompare, comparison)
{
  ...
  for (long i=0; i<=bitSize; i++) {
    if (i<bitSize) secKey.Encrypt(enca[i], NTL::ZZX((pa>>i)&1));
...
#ifdef DEBUG_PRINTOUT
  decryptAndPrint((std::cout<<" before comparison: "), encb[0], secKey, ea,0);
#endif
  ...
  decryptAndPrint((std::cout<<" after comparison: "), *minLvlCtxt, secKey, ea,0);
  std::cout << std::endl;
#endif
}
